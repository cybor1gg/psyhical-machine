...
// files): a camera rig held at one pose over a deep starfield, twinkles, the
// drifting solar system (sun video, breathing rays, corona, 7 orbiting
// planets), fast parallax stars, shooting stars and a vignette.
//
...
function SunVideo() {
  ...
  useEffect(() => {
    ...
    if (typeof v.requestVideoFrameCallback === "function") {
      ...
      raf = requestAnimationFrame(draw);
      // low-rate safety net for that path only: keeps the sun alive when
      // animation frames are throttled (background pane, minimized kiosk).
      // With rVFC it would repaint 4x/s on top of frames already painted —
      // a clear + scaled drawImage + texture upload + a forced compositor
      // frame, forever, for nothing.
      tick = setInterval(() => { if (!v.paused) paint(); }, 250);
    }
...
function SolarSystem({ anchorTop = "26%" }) {
  return (
    <div style={{ position: "absolute", left: "84%", top: anchorTop, width: 0, height: 0, pointerEvents: "none", animation: "bjSunDrift 70s ease-in-out infinite" }}>
      {/* Breathing ray shafts. NO will-change anywhere in this subtree: Gecko
          charges every declaration its border-box area against a document
          budget (viewport area x 3) and IGNORES the ones past it — and a
          running CSS animation already promotes its element, so a hint here
          would only spend budget the screens above us need. */}
  ...
  return (
    <>
      {/* Flat rig (no preserve-3d): video textures inside 3D subtrees fail
          to paint on some GPUs. The camera holds ONE pose: a sway on a frame
          this size recomposites the whole sky every frame, and its ink
          overflow (the star layers' skirts) is past the size Gecko will
          prerender, so the animation drops to the main thread anyway. */}
      <div className="fx-cam" style={{ position: "absolute", inset: "-6%", pointerEvents: "none", transform: "perspective(1100px) rotateX(5deg) rotateY(-4deg) rotateZ(-1deg) scale(1.14)" }}>
        {/* Deep starfield — drifts one tile per cycle as a GPU layer move.
            ONE deep layer and one fast layer: each is a full-screen fill, and
            past two the extra depth is not visible under the game screens. */}
        {[
          ...
          <div key={i} className={`fx-stars-deep${i}`} style={{ position: "absolute", inset: `-${L.t}px 0px 0px -${L.t}px`, pointerEvents: "none", opacity: 0.5, backgroundImage: L.img, backgroundSize: L.size, animation: `${L.anim} 16s linear infinite` }} />
        ))}
        ...
        {[
          ...
          <div key={i} className={`fx-stars-fast${i}`} style={{ position: "absolute", inset: `-${L.t}px -${L.t}px 0px 0px`, pointerEvents: "none", opacity: 0.55, backgroundImage: L.img, backgroundSize: L.size, animation: `${L.anim} ${fastDur}s linear infinite` }} />
        ))}
      </div>
